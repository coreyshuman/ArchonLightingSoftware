...
namespace ArchonLightingSystem.WindowsSystem
{
    public static class HdrManager
    {
        ...
        {
            if (WindowsSystem.WindowsVersion < WindowsVersion.Win10)
            {
                return false;
            }
            ...
            try
            {
                ...
                {
                    Logger.Write(Level.Error, "Failed to retrieve primary display target info");
                    return false;
                }
            ...
            {
                Logger.Write(Level.Error, $"Failed to check if HDR is supported: {ex.Message}");
                return false;
            }
        }
        ...
        public static bool IsHdrEnabled()
        {
            ...
            {
                Logger.Write(Level.Error, $"Failed to check if HDR is enabled: {ex.Message}");
                return false;
            }
        }
        ...
        public static void SetHdrEnabled(bool enable)
        {
            ...
            {
                Logger.Write(Level.Error, $"Failed to set HDR state: {ex.Message}");
            }
        }
        ...
        private static DISPLAYCONFIG_PATH_TARGET_INFO? GetPrimaryDisplayTargetInfo()
        {
            ...
            {
                Logger.Write(Level.Error, "Failed to retrieve display config buffer sizes");
                return null;
            }
            ...
            if (result != WinError.ERROR_SUCCESS)
            {
                Logger.Write(Level.Error, "Failed to retrieve display config info");
                return null;
            }
        ...
        private static (bool, bool) GetHdrStatus(DISPLAYCONFIG_PATH_TARGET_INFO targetInfo)
        {
            ...
            if (result != WinError.ERROR_SUCCESS)
            {
                Logger.Write(Level.Error, "Failed to retrieve advanced color info");
                return (false, false);
            }
